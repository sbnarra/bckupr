...
func Fill[T any](i *T, defaults *Defaults) error {
	return fillUsingName(fmt.Sprintf("%T", i), i, defaults)
}

func fillUsingName[T any](schemaName string, i *T, defaults *Defaults) error {
	...
	if schema.Value.Default != nil {
		el.Set(reflect.ValueOf(schema.Value.Default.(T)))
		return nil
	} else {
