...
func (d Docker) RunContainer(image string, cmd []string, env []string, volumes []string) (string, error) {
	return d.runContainer(image, cmd, env, volumes, true)
}

func (d Docker) runContainer(image string, cmd []string, env []string, volumes []string, pullIfMissing bool) (string, error) {
	if container, err := d.client.ContainerCreate(
		...
		if pullIfMissing && fmt.Sprintf("%T", err) == "errdefs.errNotFound" {
			...
			return d.runContainer(image, cmd, env, volumes, false)
		}
