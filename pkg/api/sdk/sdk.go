...
func (s *Sdk) StartBackup(ctx context.Context, req spec.TaskInput) (*spec.Backup, *errors.E) {
	...
	if res == nil {
		return nil, newError(err, "error starting backup")
	} else if checkSuccess(res.HTTPResponse, err) {
		backup := res.JSON200
		return backup, nil
	} else {
		return nil, newError(err, "error starting backup: "+string(res.Body))
	}
}
...
func (s *Sdk) StartBackupWithId(ctx context.Context, id string, req spec.TaskInput) (*spec.Backup, *errors.E) {
	...
	if res == nil {
		...
		return nil, newError(err, "error starting backup:"+id+": "+string(res.Body))
	}
}

func newError(err error, msg string) *errors.E {
	if err == nil {
...
func (s *Sdk) GetBackup(ctx context.Context, id string) (*spec.Backup, *errors.E) {
	...
	if res == nil {
		return nil, newError(err, "error finding backup")
	} else if checkSuccess(res.HTTPResponse, err) {
		backup := res.JSON200
		return backup, nil
	} else {
		return nil, newError(err, "error finding backup: "+id+": "+string(res.Body))
	}
}
...
func (s *Sdk) DeleteBackup(ctx context.Context, id string) *errors.E {
	...
	if res == nil {
		return newError(err, "error starting backup")
	} else if !checkSuccess(res.HTTPResponse, err) {
		return newError(err, "error starting backup: "+string(res.Body))
	}
	return nil
}
...
func (s *Sdk) ListBackups(ctx context.Context) ([]spec.Backup, *errors.E) {
	...
	if res == nil {
		return nil, newError(err, "error listing backups")
	} else if checkSuccess(res.HTTPResponse, err) {
		backups := res.JSON200
		return *backups, nil
	} else {
		return nil, newError(err, "error listing backups: "+string(res.Body))
	}
}
...
func (s *Sdk) StartRestore(ctx context.Context, backupId string, req spec.TaskInput) (*spec.Restore, *errors.E) {
	...
	if res == nil {
		return nil, newError(err, "error starting restore")
	} else if checkSuccess(res.HTTPResponse, err) {
		restore := res.JSON200
		return restore, nil
	} else {
		return nil, newError(err, "error starting restore: "+string(res.Body))
	}
}
...
func (s *Sdk) GetRestore(ctx context.Context, id string) (*spec.Restore, *errors.E) {
	...
	if res == nil {
		return nil, newError(err, "error finding restore")
	} else if checkSuccess(res.HTTPResponse, err) {
		restore := res.JSON200
		return restore, nil
	} else {
		return nil, newError(err, "error finding restore: "+id+": "+string(res.Body))
	}
}
...
func (s *Sdk) StartRotate(ctx context.Context, req spec.RotateInput) (*spec.Rotate, *errors.E) {
	...
	if res == nil {
		return nil, newError(err, "error starting rotate")
	} else if checkSuccess(res.HTTPResponse, err) {
		task := res.JSON200
		return task, nil
	} else {
		return nil, newError(err, "error starting rotate: "+string(res.Body))
	}
}
...
func (s *Sdk) GetRotate(ctx context.Context) (*spec.Rotate, *errors.E) {
	...
	if res == nil {
		return nil, newError(err, "error finding rotate")
	} else if checkSuccess(res.HTTPResponse, err) {
		task := res.JSON200
		return task, nil
	} else {
		return nil, newError(err, "error finding rotate: "+string(res.Body))
	}
}
...
func (s *Sdk) Version(ctx context.Context) (*spec.Version, *errors.E) {
	...
	if res == nil {
		return nil, newError(err, "error getting version")
	} else if checkSuccess(res.HTTPResponse, err) {
		task := res.JSON200
		return task, nil
	} else {
		return nil, newError(err, "error getting version:"+string(res.Body))
	}
}
